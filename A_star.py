...
def find_children_nodes(node, heuristic_number):
    ...
    open_list.sort(key=lambda x: (x.get_h() + x.get_g()))
    temp_configuration = []
    temp_list = []
    ...
    print('start point')
    for i in range(len(open_list)):
        print(open_list[i].get_configuration(),(open_list[i].get_h())+open_list[i].get_g())
    print("break point")
...
def apply_algorithm(start_node, heuristic_number):
    ...
    while (open_list):
        ...
        search_file_data.append((
                                current_node.get_g() + current_node.get_h(), current_node.get_g(), current_node.get_h(),
